...
"""
            ...
            if False:
                for names_list in names_lists:
                    names_list['list']=sorted(names_list['list'], key=lambda x: (x['y_2'], x['x_1']))

                    print(names_list['page'])
                    # names_list['list']=sorted(names_list['list'], key=lambda x: (x['record']['text'].lower()))
                    for name in names_list['list']:
                        relics=[]
                        index=""
                        if 'list_index_record' in name:
                            p=self.get_record(gid=name['list_index_record'][0])
                            index=p['list_index']
                            relics.append(index)
                        ...
                        ipen=""
                        ipen_dist=""
                        if 'ipen_record' in name:
                            p=self.get_record(gid=name['ipen_record'][0])
                            ipen=p['ipen']
                            relics.append(ipen)
                            ipen_dist=name['ipen_record'][1]

                        sp_name=self.clean_up_name_string(name['text'], relics=relics)
                        # sp_name=name['record']['text']
                        match=name['species_match']
                        
                        # sp_dist=name['dist']
                        # sp_seq=name['seq']
                        
                        print(f"{index:>5} {sp_name:<60} {ipen:<25} {match:<15}")
                    print()
"""